...
Entry points come from three signals: script tables in project manifests
(``pyproject.toml`` / ``package.json``), well-known main-file basenames, and
files that define a top-level ``main()`` function in a language where that is
the program entry point (C, C++, Go, Rust).
"""
...
def entry_points(report: Report) -> list[EntryPoint]:
    """Return detected entry points, manifests first, capped at 10."""
